...
This is a template service for express-typescript-openapi3, which uses TypeScript, Express, OpenAPI3.0, AWS Serverless, Aurora Serverless.

## Instruction

### Initial setup

1. Copy template directory, rename root directory name.
2. Run "npm install" once to get necessary node modules.
3. Replace the word "template-service" in package.json.
4. Replace the word "template-service" in serverless.yml.

### Develop your codes

1. src/controllers/YOUR_CONTROLLER.ts
2. src/definition/swagger.yaml
3. src/inversify/interfaces.ts, src/inversify/inversify.config.ts, src/types.ts
4. src/repositories/YOUR_REPOSITORY.ts
5. src/routes/YOUR_ROUTE.ts
6. src/serializers/YOUR_SERIALIZER.ts
7. test code

### Write document Readme.md

1. Summary of a microservice
2. Architecture diagram
3. Links, important infomation and references
4. Onboarding, Development rule and guide
5. Request flow
6. Endpoints (OpenAPI 3.0)
7. Dependencies
8. On call run book（optional)
9. FAQ (optional)
...
- make PR from master to deploy-template-service in GitHub, merge it.
...
- this feature is currently disabled in swagger.ts. because Lambda can write only to /tmp directory, although oas-tools write a file to other directory by writeFileSync to write swagger.yml.
...
- if it's enabled API Document endpoints, you can see 
